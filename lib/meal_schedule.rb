class MealSchedule
  class << self
    def upcoming_meals_and_suggestions(days:, from_date: Date.today, user:)
      to_date = end_date_of_schedule(days: days, from: from_date)
      scheduled_meals = scheduled_upcoming_meals(from_date: from_date, to_date: to_date, user: user)
    ...
    def scheduled_upcoming_meals(from_date: Date.today, to_date: Date.today, user:)
      ScheduledMeal
        .joins(:meal)
    ...
    def suggest_meals(number_of_meals, user)
